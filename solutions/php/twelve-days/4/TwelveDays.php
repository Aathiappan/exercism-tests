...
class TwelveDays
{
    private const SECOND_HALF_PARTS = [1 => 'a Partridge in a Pear Tree.',
    ...
    private const FORMATTER = [1 => 'first', 2 => 'second', 3 => 'third', 4 => 'fourth', 5 => 'fifth', 6 => 'sixth', 7 => 'seventh', 8 => 'eighth', 9 => 'ninth', 10 => 'tenth', 11 => 'eleventh', 12 => 'twelfth'];
    
    public function recite(int $start, int $end): string
    {
        $verse = '';
        for($line = $start;$line <= $end;$line++){
            $line_words = self::FORMATTER[$line];
            $verse .= "On the $line_words day of Christmas my true love gave to me: ";
            if($line === 1){
                $verse .= self::SECOND_HALF_PARTS[1];
            }else{
                //$parts = range($line,1);
                $verse .= implode(', ', array_map(
                    fn ($part) => self::SECOND_HALF_PARTS[$part],
                    range($line, 2),
                ));
                $verse .= ', and '.self::SECOND_HALF_PARTS[1];
            }
            if($line !== $end){
                $verse .= PHP_EOL;
            }
