...
class RobotSimulator
{
    ...
    */
    ...
    {
        $this->robotPos = [$position[0],$position[1]];
        $this->angle = $this->dirAngle[$direction];
    }
    ...
    public function instructions(string $instructions): void
    {
        if(strlen($instructions) === 1){
            if($instructions != 'A'){
                $this->angle = $this->getAngle($instructions);
            }else{
                $this->move();
            }
    ...
    {
        return array_search($this->angle,$this->dirAngle);
    }
    private function getAngle(string $direction):int{
        $initial = $this->angle;
